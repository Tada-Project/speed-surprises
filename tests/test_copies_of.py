...
@given(
    copies_as_int=integers(min_value=1),
    chosen_letter=text(min_size=1, max_size=1))
...
def test_letter_count_hypothesis(copies_as_int, chosen_letter):
    ...
    copied_character_string = copies.mcopies_of(copies_as_string,
                                                chosen_letter)
    assert len(copied_character_string) == copy_count
    assert copied_character_string.count(chosen_letter) == copy_count


@pytest.mark.parametrize("letter_count,expected_count,chosen_letter", [
    ("1", 1, CHOSEN_LETTER),
    ("10", 10, CHOSEN_LETTER),
